...
import { getCached, putCached, getBatchCached, putBatchCached, isCacheValid } from "./cache.js";
...
import { DISABLE_CACHE_MODE, DISABLE_WRITES, CACHE_SCHEMA_VERSION } from "./config.js";
import { MAIA_THREE_PATH, ELITE_LEELA_PATH, LEELA_PATH } from "./config.js";
...
export class ModelLoader {
  ...
  async analyzeMaia3(fen: string, rating: number, rawWDL = false): Promise<EngineAnalysis> {
    ...
    const hit = !DISABLE_CACHE_MODE ? await getCached(cacheNet, fen) : null;

    if (hit && isCacheValid(hit, rawWDL)) {
      return hit as EngineAnalysis;
    }
    ...
    if (!DISABLE_CACHE_MODE && !DISABLE_WRITES) {
  ...
  async batchAnalyzeMaia3AllLevels(fen: string): Promise<{ rating: number; analysis: EngineAnalysis }[]> {
    const cached = !DISABLE_CACHE_MODE ? await getBatchCached(fen) : null;
    if (cached) {
      return cached;
    };
